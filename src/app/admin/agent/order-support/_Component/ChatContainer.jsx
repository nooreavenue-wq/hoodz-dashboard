...
export default function ChatContainer() {
  ...
  // Sync HTTP chat list → only order_support
  useEffect(() => {
    const list = listRes?.data?.order_support || [];
    setChatList(list);
  }, [listRes]);
  ...
  useEffect(() => {
    ...
    const onList = (payload) => {
      // payload may be full object or { order_support: [...] }
      const list =
        payload?.order_support ||
        payload?.data?.order_support ||
  ...
  const otherUser = useMemo(() => {
    ...
    const orderId = activeChat?.order?.id || null;
      ...
      <div className="flex w-full max-w-[340px] flex-col border-r border-slate-100">
        <div className="border-b border-slate-100 p-4">
          <div className="mb-3 flex items-center gap-2">
            ...
            <h2 className="text-lg font-semibold">Order Support</h2>
          </div>
          <Input
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        {!activeChatId ? (
          ...
          <ChatWindow
            ...
            orderId={activeChat?.order?.id || null}
          />
        )}
      </div>
    </div>
